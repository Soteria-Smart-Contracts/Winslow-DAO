...
contract CLDAuctionFactory {
    ...
    function newCLDAuction( uint256 _EndTime, uint256 _AmountToAuction, uint256 _MinimunFeeInGwei, uint256 _RetireeFeeInBP, address payable[] memory _DevTeam ) external OnlyDAO returns(address ContractAddress){
    ...
    function SeeAuctionData(uint256 AuctionID) public view returns (address, uint256, uint256, uint256){
        return (auctionList[AuctionID].auctionAddress, auctionList[AuctionID].startDate, auctionList[AuctionID].endDate, auctionList[AuctionID].amountAuctioned );
    }
}
...
contract CLDAuction {
    ...
    constructor( uint256 _StartTime, uint256 _EndTime, uint256 _Amount, uint256 _MinimunFeeInGwei, uint256 _RetireeFeeInBP, // BP = Basis points (100 (1%) to 10000 (100%)) address _DAO, address payable _Treasury, address _CLD ) {
