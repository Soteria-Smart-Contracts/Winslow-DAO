...
contract Winslow_Core_V1 {
    ...
    function InitializeSimpleProposal(string memory Memo, address AddressSlot, uint256 UintSlot, SimpleProposalTypes SimpleType, uint256 VotingLength, uint256 RequestedEther, uint256 RequestedAssetAmount, uint8 RequestedAssetID) internal returns(uint256 Identifier){
        ...
        if(SimpleType == SimpleProposalTypes(2) || SimpleType == SimpleProposalTypes(3)){
            ...
            ProposalInfos[NewIdentifier] = ProposalInfo(ProposalTypes(0), SimpleType, ProposalStatus(0), VotingInstanceID, VotingLength);
            Proposals[NewIdentifier] = Proposal(AddressSlot, SecurityStatus(0), RequestedEther, RequestedAssetAmount, RequestedAssetID, 0, false, false, msg.sender);
    ...
    function InitializeProxyProposal(string Memo, address Slot, uint256 VotingLength, uint256 RequestedEther, uint256 RequestedAssetAmount, uint8 RequestedAssetID, ProxyProposalArguments memory ProxyArguments) internal returns(uint256 identifier){
    ...
    function InitializeErosProposal(string Memo, address Slot, uint256 VotingLength, uint256 RequestedEther, uint256 RequestedAssetAmount, uint8 RequestedAssetID) internal returns(uint256 identifier){
