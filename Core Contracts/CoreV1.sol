...
//How does it know what to call for Simple proposal? set up system, maybe some kind of int parser
    ...
    function SubmitSimpleProposal(address Slot, SimpleProposalTypes SimpleType, uint256 VotingLength, uint256 RequestedEther, uint256 RequestedAssetAmount, uint8 RequestedAssetID) public returns(bool success, uint256 Identifier){
        ...
        InitializeSimpleProposal(NewIdentifier, Slot, SimpleType, VotingLength, RequestedEther, RequestedAssetAmount, RequestedAssetID);
...
//    }
    ...
    function InitializeProxyProposal(uint256 NewIdentifier, address Slot, string memory Memo, uint256 VotingLength, uint256 RequestedEther, uint256 RequestedAssetAmount, uint8 RequestedAssetID) internal returns(uint256 identifier){
        require(VotingLength >= 86400 && VotingLength <= 1209600, "Voting must be atleast 24 hours and less than two weeks");
