...
//How does it know what to call for Simple proposal? set up system, maybe some kind of int parser
    ...
    function InitializeSimpleProposal(uint256 NewIdentifier, SimpleProposalTypes Type, uint256 VotingLength, uint256 RequestedEther, uint256 RequestedAssetAmount, uint8 RequestedAssetID) internal returns(uint256 Identifier){
        ...
        Proposal memory NewProposal = Proposal(NewIdentifier, ProposalStatus(0), SecurityStatus(0), 0, 0, VotingLength, RequestedEther, RequestedAssetAmount, RequestedAssetID, EmptyProxy, false);
