...
contract Winslow_Voting_V1 {
    ...
    event InstanceCreated(address Proposer, address ProposalID, address block.timestamp, block.timestamp + Time)
    ...
    function InitializeVoteInstance(uint256 ProposalID, uint8 MaxMulti) external OnlyDAO returns(uint256 VoteInstanceID){
        ...
        emit InstanceCreated(Proposer, ProposalID, block.timestamp, block.timestamp + Time);
