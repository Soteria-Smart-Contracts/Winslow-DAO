...
//When setting up a new core or voting contract, ensure cross-compatibility and record keeping done by the archive contract, voting index and proposal indexes 
...
contract VotingSystemV1 {
    ...
    function CastVote(uint256 amount, uint256 VotingInstance, Vote VoteChoice) external returns(bool success){
        ...
        if(VoteChoice == Vote(0)) {
            ...
            emit VoteCast(VotingInstance, "Yes", amount);
        } else {
            VotingInstances[VotingInstance].NAYvotes += amount;
            emit VoteCast(VotingInstance, "No", amount);
        }
    ...
    function InitializeVoteInstance(address Proposer, uint256 ProposalID, uint256 Time) external OnlyDAO {
        ...
        VotingInstances.push(VoteInstance(ProposalID,0,0,VoteStatus(0),0,0,0,false,0,0,0,0,0));
