...
contract Winslow_Voting_V1 {
    ...
    function CastVote(uint256 amount, uint256 VotingInstance, Vote VoteChoice) external returns(bool success){
        ...
        if(VotingInstances[VotingInstance].Voters == 0){
            VotingInstances[VotingInstance].Status = Voting;
        }
    ...
    function InitializeVoteInstance(uint256 ProposalID, uint256 VotingLength, bool Multi) external OnlyDAO returns(uint256 VoteInstanceID){
        ...
        VotingInstances[NewInstanceID] = VoteInstance(ProposalID,PublicVotingStart,PublicVotingEnd,VoteStatus(0),0,0, Multi,0,0,0,0,0,0,0);
