...
contract ErosProposal {
    ...
    modifier OnlyDAO() {
        //This same modifier must be used on external contracts called by this contract
    ...
    function Execute() external OnlyDAO returns (bool success) {
        ...
        if (address(this).balance > 0) {
            //Must be the last state changing part of this function
...
interface ExtCon {
    //Interface name can be different, ensure it is updated correctly with the external functions to be used in execution
